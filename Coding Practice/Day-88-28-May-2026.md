...
- name: Setup VPC Peering Between Accounts
  ...
  tasks:
    - name: Create VPC peering connection
      connection: aws ec2 create-vpc-peering-connection --vpc-id vpc-123455678 --peer-vpc-id vpc-87654321
...
apiVerison: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
