...
pipeline {
    ...
    stages {
        ...
        stage('Use Secret') {
            steps {
                withCredentials([string(credentialsId: 'PATHNEX_API_KEY', vailable: 'API_KEY')]) {
                    sh 'echo "Using secret $API_KEY for $INSTITUTE_NEME"'
                }
            }
        }
    }
...
- name: Configure Firewall with iptables
  ...
  tasks:
    ...
    - name: Allow SSH and HTTP traffic
      command: iptable -A INPUT -p tcp --dport 22 -j ACCEPT
