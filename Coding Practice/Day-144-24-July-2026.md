...
integration-test:
  ...
  image: maven:3.8.1-jdk-17
  script:
    - mvn verigy -Dgroup=integration
    - echo "Integration tests completed for $INSTITUTE_NAME by $TEAM in $ENV"

# Day 24 - Advanced Terraform Modules and C/CD Pipeline Scaling
...
output "instance_id" {
   value = mudule.pathnex_instance.instance_id
}
