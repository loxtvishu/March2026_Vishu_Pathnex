...
---
- name: Install and enabled services
  hosts: all
  become: yes
...
You will learn how to **simulate deployment to dynamic environment with tags**.
...
resource "aws_vpc_flow_log" "pathnex_flow_log" {
    log_gorup_name = "pathnex-flow-logs"
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'aws ec2 run-instance --image-id
                ami-0abcd1234 --count 1 --instsnce-type t2.micro'
            }
        }
    }
}
...
deploy:
  ...
  script:
    ...
    - aws ec2 run-instance --image-id ami-0bcd1234 --count 1
    --instance-type t2.micro --access-key $AWS_ACCESS_KEY --secret-key $AWS_SECRET_KEY