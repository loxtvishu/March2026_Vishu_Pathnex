...
pupeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                sh '''
                docker build -t pathnex/webapp: ${ENV}-${BUILD} \
...
- name: Deploy AWS Lambda Function
  ...
  tasks:
     -name: Create Lambda function
      command: aws lambda create-function --function-name pathnex-lambda --runtime python3.8 --role arn:aws:iam::123456789012:role/lambda-execution-role --handler lambda-execution-role --handler lambda_function.lambda_handler --zip-file fileb://lambda.zip
    ...
    stages {
        stage('Build') {
            steps {
                ...
                sh 'zip -r lambda.zip lamda/'
            }
        }
        stage('Deploy to AWS Lambda') {
            steps {
                script {
                    sh 'aws lamda update-function-code --function-name
                    pathnex-lambda --zip-file fileb://lambda.zip'
                }
            }
        }
    }
