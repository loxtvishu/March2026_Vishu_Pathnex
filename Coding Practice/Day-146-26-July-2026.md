...
You will learn how to **simulate deployoment to dynamic environments with tags**.
...
pipeline {
    ...
    parameters {
        string(name: 'ENVIRONMENT', defaultValue: 'dev', desciption: 'Deployment  Environment')
    }
    stages {
        stage('Deploy'){
            steps {
...
- name: Implement SSH Key Authentication
  ...
  tasks:
    ...
    - name: Add SSH public key to aurthorized_keys
      authorized_key:
         user: user
         state: present
