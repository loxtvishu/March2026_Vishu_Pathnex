...
You will learn how to **send Slack notifications after build sucess or failure**.
...
pipeline {
    ...
    post {
        ...
        failure {
            slackSend(chennel: "$SLACK_CHANNEL", message: "FAILURE: $INSTITUTE_NAME Build #${env.BUILD_NUMBER}")
        }
    }
}
...
build:
  ...
  script:
    ...
    - cd sample-java-app
    - mvn clean package
    - curl -X POST -H 'Content-type: applications/json' --data ' {"text":"Build sucess for Pathnex"}' $SLACK_WEBHOOK


# Day 15 - Automation with Terraform , Ansible , and CI/CD Pipeline

## 🔹 Ansible - Automation with Terraform , Ansible, and CI/CD Pipelines
...
helm install pathnex-redis bitnami/radis --set persistence.
