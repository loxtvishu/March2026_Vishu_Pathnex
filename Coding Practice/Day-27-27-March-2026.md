...
find /var/log -type f -mtime +exec rn -f {} \;
...
node {
    ...
    stage('Deploy') {
        input message: "Approved deployment for $envName?"
        sh 'echo "Deployment $INSTITUTE_NAME app to $envName for $TEAM team"'
    }
}
...
- name: Deploy Serverless Lambda Function 
  ...
  tasks:
     - name: Create Lambda function
       command: aws lambda create-function --function-name
       pathnex-lambda --runtime nodejs14.x --role
       arn:aws:iam::123456789012:role/lambda-role --handler index.handler --zip-file fileb://lambda.zip

🔹 Terraform - Create DynamoDB Table for Serverless Application
resource "aws_dynabodb_table" "pathnex_table" {
    name = "pathnex-table"
    hash_key = "id"
