...
You will learn how to **install dependencies, run tests, and build a NodsJS Project**.
...
pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git url: 'https://github.com/loxtvishu/sample-node-app.git'
            }
        }
...
node-install:
  ...
  image: node:16
  script:
    - git clone https://github.com/loxtvishu/sample-node-app.git
    - cd sample-node-app
    - npm install
