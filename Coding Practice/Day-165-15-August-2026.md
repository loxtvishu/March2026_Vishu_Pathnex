...
build:
  ...
  script:
    ...
    - cd sample-java-app
    - mvn clean package
    - curl -X POST -H 'Content-type: application/json' --data '{"text":"Build Sucess for Pathnes"}' $SLACK_WEBHOOK
...
helm install pathnex-redis bitnami/redis --set persistence.enabled=true --set persistence.
size=8Gi
...
## 🔹 GitLab CI/CD - Environmnet-Specific Depoyments
  ...
  script:
    - |
      if [ "$CI_COMMIT_REF_NAME" == "main" ]; then
          ...
          kubectl apply -f kubenetes/dev-deployment.yaml
      fi
