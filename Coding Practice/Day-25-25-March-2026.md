...
## 🔹 Ansible - Setup Prometheus Node Exporter for Monitering
...
resource "aws_cloudwatch_metric_alarm" "pathnex_cpu_alarm" {
   alarm_name = "pathnex-cpu-utilization-hign"
   comparison_operator = "GreaterThanThreeshold"
...
pipeline {
    ...
    stages {
         stage('Monitor') {
             steps {
                 script {
                    echo 'Triggering CloudWatch alarms if CPU exceds 80%'
